...
//         }];
        ...
        //[self.callInvite acceptWithOptions:acceptOptions delegate:self];
        //self.call = self.callInvite.accept(with: options, delegate: self)
    }
...
- (void)handleCallInviteReceived:(TVOCallInvite *)callInvite {
    ...
    if (self.callInvite == nil && self.call == nil) {
        ...
        NSLog(@"Call Invite Received During Call. Ignoring: %@", self.callInvite);
        NSLog(@"Call Invite Received During Call. Ignoring: %@", self.call);
    }
}
